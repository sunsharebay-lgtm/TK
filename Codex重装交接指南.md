...
- 项目实际路径：`/Users/sun/Desktop/pgcs/创意空间`
...
- 本地预览：`cd "/Users/sun/Desktop/pgcs/创意空间" && python3 -m http.server 8123 --bind 127.0.0.1`，再访问 `http://127.0.0.1:8123/创意空间首页/`。
...
6. 改动推送到 `main` 会自动触发 GitHub Pages 重新部署；发布稳定版用 `game/<id>/vX.Y.Z` 标签。
...
- 粉丝资源：`resources.json`（手动维护，`resources` 数组加条目；每条使用固定两位 `number`，格式见本项目 `更新记录.md` 与 `project.md`）。
- 长资料独立页：`资源/<资料名>/`，首页资源列表为一行式，每条必须提供 `detailUrl`；当前资源为 `资源/giffgaff保命资料/`。
- 测试：`测试/game_center_test.js`、`测试/version_catalog_test.js`、`测试/resources_test.js`。
...
- 运行测试：`node 创意空间首页/测试/game_center_test.js && node 创意空间首页/测试/version_catalog_test.js && node 创意空间首页/测试/resources_test.js`
...
- 运行：`node 超级像素兄弟/test/harness.js`
- 入口：`超级像素兄弟/index.html`
...
- 部署工作流 `.github/workflows/pages.yml`：推送 `main` 或 `game/**/v*` 标签时运行生成脚本并部署。
- 标签规范：坦克大战 `game/tank-battle/vX.Y.Z`；三国 RPG `game/three-kingdoms/vX.Y.Z`；超级像素兄弟 `game/super-pixel-brothers/vX.Y.Z`。不要创建全局旧式标签。
...
1. 打开项目：`cd "/Users/sun/Desktop/pgcs/创意空间"`
