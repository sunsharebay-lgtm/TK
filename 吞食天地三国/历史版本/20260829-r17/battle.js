...
class Scene_Battle {
  ...
  doActorAction(a) {
    ...
    if (item.id === 1) {                       // 普通攻击
      ...
      const t = act.target && !act.target.isDead() ? act.target : this.defaultTargetFor(item, a);
      if (!t) return;
      this.actionVisual = { user: a, target: t, hitT: 0 };
      a.paySkillCost(item);
  ...
  defaultTargetFor(item, user = this.subject) {
    ...
    if ([1, 2, 9].includes(scope)) return enemyUser ? this.pickRandomAliveActor() : this.pickRandomAliveEnemy();
    return this.subject;
  }
